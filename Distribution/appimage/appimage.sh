...
BINFILE=Project_M_Reload_Dolphin-x86_64.AppImage
...
mkdir -p squashfs-root/usr/share/icons/hicolor/scalable/apps && cp ./squashfs-root/project-m-reload-dolphin.svg ./squashfs-root/usr/share/icons/hicolor/scalable/apps
mkdir -p squashfs-root/usr/share/pixmaps && cp ./squashfs-root/project-m-reload-dolphin.svg ./squashfs-root/usr/share/pixmaps
mkdir -p squashfs-root/usr/optional/ ; mkdir -p squashfs-root/usr/optional/libstdc++/

mkdir -p squashfs-root/usr/share/project-m-reload-dolphin
cp -R /dolphin/dolphin/Data/Sys ./squashfs-root/usr/bin
curl -sL "https://raw.githubusercontent.com/AeonSSB/Project-M-Reload-Dolphin/$branch/Distribution/appimage/update.sh" -o $HOME/squashfs-root/update.sh
curl -sL "https://raw.githubusercontent.com/AeonSSB/Project-M-Reload-Dolphin/$branch/Distribution/appimage/AppRun" -o $HOME/squashfs-root/AppRun
...
# Package AppImage
/tmp/squashfs-root/usr/bin/appimagetool $HOME/squashfs-root -u "gh-releases-zsync|qurious-pixel|dolphin|continuous|Project_M_Reload_Dolphin-x86_64.AppImage.zsync"

mkdir $HOME/artifacts/
mkdir -p /dolphin/artifacts/
mv Project_M_Reload_Dolphin-x86_64.AppImage* $HOME/artifacts
cp -R $HOME/artifacts/ /dolphin/
cp $BUILDBIN/project-m-reload-dolphin /dolphin/artifacts
